...
describe 'User tries to bid' do
  it 'successfully' do
    ...
    Batch.create!(code: '123asd456', start_date: Date.new(2023, 5, 18), final_date: 1.day.from_now, minimum_value: 200,
                  minimum_difference: 50, items: [], approved: true, created_by_id: Admin.first.id, approved_by_id: Admin.last.id)
  ...
  it "and can't bid" do
    ...
    Item.create!(name: 'Caneca HP', description: 'Caneca do harry potter', weight: 300, width: 10, height: 20,
                 depth: 16, category_id: Category.first.id)
    Batch.create!(code: '125bsd456', start_date: Date.new(2023, 5, 20), final_date: 1.day.from_now, minimum_value: 200,
                  minimum_difference: 50, items: [Item.first], approved: true, created_by_id: Admin.last.id, approved_by_id: Admin.first.id)
